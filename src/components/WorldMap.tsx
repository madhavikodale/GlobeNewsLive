...
import { NUCLEAR_SITES, SPACEPORTS, AI_DATA_CENTERS, IRAN_TARGETS, UNDERSEA_CABLES, PIPELINES } from '@/lib/infrastructure';
import { TRADE_HUBS, TRADE_ROUTES, getRoutesGeoJSON, getHubsGeoJSON, CLUSTER_REGIONS } from '@/lib/trade-routes';
...
function calculateClusters(signals: Signal[], earthquakes: Earthquake[], conflicts: typeof ACTIVE_CONFLICTS) {
  const clusters: { region: string; lat: number; lon: number; count: number; color: string }[] = [];
  
  // Pre-defined cluster regions with counts
  const regionCounts: Record<string, { count: number; lat: number; lon: number }> = {
  ...
  signals.forEach(s => {
    ...
    if (text.includes('ukrain') || text.includes('russia') || text.includes('europe') || text.includes('nato')) {
      regionCounts['EUROPE'].count++;
    } else if (text.includes('iran') || text.includes('israel') || text.includes('gaza') || text.includes('syria') || text.includes('yemen') || text.includes('iraq') || text.includes('lebanon')) {
      regionCounts['MENA'].count++;
    } else if (text.includes('china') || text.includes('taiwan') || text.includes('korea') || text.includes('japan') || text.includes('india') || text.includes('asia')) {
      regionCounts['ASIA'].count++;
    } else if (text.includes('us ') || text.includes('america') || text.includes('canada') || text.includes('mexico')) {
      regionCounts['NORTH AMERICA'].count++;
    } else if (text.includes('brazil') || text.includes('argentina') || text.includes('venezuela')) {
      regionCounts['SOUTH AMERICA'].count++;
    } else if (text.includes('africa') || text.includes('nigeria') || text.includes('sudan') || text.includes('ethiopia')) {
  ...
  earthquakes.forEach(eq => {
    if (eq.lon > -30 && eq.lon < 60 && eq.lat > 35 && eq.lat < 70) regionCounts['EUROPE'].count++;
    else if (eq.lon > 20 && eq.lon < 70 && eq.lat > 10 && eq.lat < 45) regionCounts['MENA'].count++;
    else if (eq.lon > 60 && eq.lon < 150 && eq.lat > -10 && eq.lat < 55) regionCounts['ASIA'].count++;
    else if (eq.lon > -170 && eq.lon < -50 && eq.lat > 15 && eq.lat < 75) regionCounts['NORTH AMERICA'].count++;
    else if (eq.lon > -90 && eq.lon < -30 && eq.lat > -60 && eq.lat < 15) regionCounts['SOUTH AMERICA'].count++;
    else if (eq.lon > -20 && eq.lon < 55 && eq.lat > -35 && eq.lat < 35) regionCounts['AFRICA'].count++;
    else if (eq.lon > 100 && eq.lon < 180 && eq.lat > -50 && eq.lat < -5) regionCounts['OCEANIA'].count++;
  });
  
  // Add conflicts by location
  conflicts.forEach(c => {
    if (c.lon > -30 && c.lon < 60 && c.lat > 35 && c.lat < 70) regionCounts['EUROPE'].count += 5;
    else if (c.lon > 20 && c.lon < 70 && c.lat > 10 && c.lat < 45) regionCounts['MENA'].count += 5;
    else if (c.lon > 60 && c.lon < 150 && c.lat > -10 && c.lat < 55) regionCounts['ASIA'].count += 5;
  ...
  Object.entries(regionCounts).forEach(([region, data]) => {
    if (data.count > 0) {
      const color = data.count > 50 ? '#ff0000' : data.count > 20 ? '#ff6600' : data.count > 10 ? '#ffaa00' : '#00ff88';
      clusters.push({
        region,
        lat: data.lat,
        lon: data.lon,
...
export default function WorldMap({ signals, activeLayers, onLayerToggle, earthquakes = [] }: WorldMapProps) {
  ...
  // Fetch flight data with real-time updates (10 second refresh)
  const { data: flightData, isLoading: flightsLoading } = useSWR<{ flights: Aircraft[]; aircraft: Aircraft[]; military: number }>(
    activeLayers.includes('flights') ? '/api/flights?region=middleeast&military=false' : null,
    fetcher,
    { refreshInterval: 10000, revalidateOnFocus: true, revalidateOnMount: true }
  );
  ...
  // Calculate clusters
  const clusters = useMemo(() => calculateClusters(signals, earthquakes, ACTIVE_CONFLICTS), [signals, earthquakes]);
      ...
      if (map.current) {
        ...
        map.current.addLayer({
          ...
          paint: {
            'line-color': [
              'case',
              ['==', ['get', 'risk'], 'critical'], '#ff4444',
              ['==', ['get', 'risk'], 'high'], '#ff8800',
              ['==', ['get', 'active'], false], '#444444',
              '#00d4ff'
            ],
            'line-width': [
  ...
  useEffect(() => {
    ...
    try {
      map.current.setLayoutProperty('trade-routes-line', 'visibility', visibility);
      map.current.setLayoutProperty('trade-hubs-circle', 'visibility', visibility);
      map.current.setLayoutProperty('trade-hubs-label', 'visibility', visibility);
    } catch (e) {
      // Layer might not exist yet
    }
  }, [activeLayers, loaded]);

  // Update flight markers separately (real-time updates)
  useEffect(() => {
    if (!map.current || !loaded) return;
    ...
    if (activeLayers.includes('flights') && flights.length > 0) {
      flights.forEach(flight => {
        ...
        const marker = new maplibregl.Marker({ element: el })
          ...
          .setPopup(new maplibregl.Popup({ offset: 25, className: 'dark-popup' })
            .setHTML(`
  ...
  useEffect(() => {
    ...
    if (activeLayers.includes('earthquakes')) {
      earthquakes.forEach(eq => {
        const color = eq.magnitude >= 6 ? '#ff2244' : eq.magnitude >= 5 ? '#ff6633' : '#aa66ff';
        const size = Math.min(1.2, 0.6 + (eq.magnitude - 4) * 0.15);
        const el = createMarkerElement('🌍', color, `M${eq.magnitude} ${eq.place}`, size);
    ...
    if (activeLayers.includes('nuclear')) {
      NUCLEAR_SITES.forEach(site => {
        const color = site.risk === 'critical' ? '#ff0000' : site.risk === 'high' ? '#ff4444' : '#ffaa00';
    ...
    if (activeLayers.includes('spaceports')) {
      SPACEPORTS.forEach(port => {
        const color = port.risk === 'critical' ? '#ff0000' : port.risk === 'high' ? '#ff6633' : '#8844ff';
    ...
    if (activeLayers.includes('iran')) {
      IRAN_TARGETS.forEach(target => {
        const color = target.risk === 'critical' ? '#ff0000' : target.risk === 'high' ? '#ff4444' : '#ff8800';
        const icon = target.type === 'nuclear' ? '☢️' : target.type === 'missile' ? '🚀' : target.type === 'airbase' ? '✈️' : '🎯';
    ...
    if (activeLayers.includes('cables')) {
      UNDERSEA_CABLES.forEach(cable => {
        [cable.points[0], cable.points[cable.points.length - 1]].forEach((pt, i) => {
      ...
      PIPELINES.forEach(pipe => {
        pipe.points.forEach((pt, i) => {
          if (i === 0 || i === pipe.points.length - 1) {
            const color = pipe.status === 'sabotaged' ? '#ff0000' : pipe.status === 'critical chokepoint' ? '#ffaa00' : '#ff8800';
    ...
    if (activeLayers.includes('ports')) {
      const STRATEGIC_PORTS = [
        { name: 'Port of Shanghai', lat: 31.2, lon: 121.5, throughput: '47M TEU', importance: 'critical' },
        { name: 'Port of Singapore', lat: 1.26, lon: 103.82, throughput: '37M TEU', importance: 'critical' },
        { name: 'Port of Rotterdam', lat: 51.95, lon: 4.13, throughput: '15M TEU', importance: 'high' },
        { name: 'Port of Dubai (Jebel Ali)', lat: 24.98, lon: 55.06, throughput: '14M TEU', importance: 'high' },
        { name: 'Port of Busan', lat: 35.1, lon: 129.07, throughput: '22M TEU', importance: 'high' },
        { name: 'Port of Los Angeles', lat: 33.74, lon: -118.27, throughput: '10M TEU', importance: 'high' },
        { name: 'Port of Hamburg', lat: 53.54, lon: 9.98, throughput: '8M TEU', importance: 'medium' },
        { name: 'Port of Hormuz (Bandar Abbas)', lat: 27.18, lon: 56.27, throughput: 'Chokepoint', importance: 'critical' },
        { name: 'Strait of Malacca (Port Klang)', lat: 2.99, lon: 101.38, throughput: 'Chokepoint', importance: 'critical' },
        { name: 'Suez Canal (Port Said)', lat: 31.26, lon: 32.31, throughput: 'Chokepoint', importance: 'critical' },
        { name: 'Port of Aden', lat: 12.78, lon: 45.03, throughput: 'Contested', importance: 'high' },
        { name: 'Port of Haifa', lat: 32.82, lon: 34.99, throughput: '1.4M TEU', importance: 'high' },
        { name: 'Port of Novorossiysk', lat: 44.72, lon: 37.77, throughput: 'Sanctioned', importance: 'medium' },
        { name: 'Port of Vladivostok', lat: 43.11, lon: 131.88, throughput: '1.5M TEU', importance: 'medium' },
        { name: 'Port of Istanbul (Bosphorus)', lat: 41.01, lon: 29.01, throughput: 'Chokepoint', importance: 'critical' },
      ];
      STRATEGIC_PORTS.forEach(port => {
        const color = port.importance === 'critical' ? '#00ccff' : port.importance === 'high' ? '#ffaa00' : '#00ff88';
        const el = createMarkerElement('⚓', color, port.name, port.importance === 'critical' ? 1.0 : 0.8);
        const marker = new maplibregl.Marker({ element: el })
          .setLngLat([port.lon, port.lat])
          .setPopup(new maplibregl.Popup({ offset: 25, className: 'dark-popup' })
            .setHTML(`<div class="text-white"><strong>⚓ ${port.name}</strong><br/>
    ...
    if (activeLayers.includes('trade-routes')) {
      const MAJOR_ROUTES = [
        { name: 'Asia-Europe (Suez)', points: [[31.2, 121.5], [1.26, 103.82], [27.18, 56.27], [31.26, 32.31], [51.95, 4.13]], color: '#ffcc00' },
        { name: 'Trans-Pacific', points: [[35.1, 129.07], [33.74, -118.27]], color: '#00ccff' },
        { name: 'Trans-Atlantic', points: [[51.95, 4.13], [40.67, -74.04]], color: '#00ff88' },
        { name: 'Cape of Good Hope', points: [[31.2, 121.5], [-33.9, 18.42], [51.95, 4.13]], color: '#ff8800' },
        { name: 'Northern Sea Route', points: [[59.94, 30.32], [64.54, 40.52], [69.65, 18.96], [77.53, 23.6], [82.5, 55.0]], color: '#aa44ff' },
      ];
    ...
    if (activeLayers.includes('gps-jamming')) {
      const GPS_JAMMING_ZONES = [
        { name: 'Eastern Mediterranean', lat: 35.5, lon: 34.5, radiusKm: 220, severity: 'critical', source: 'Russia/Syria', description: 'Persistent GPS/GNSS jamming from Russian forces in Syria. Affects civilian aviation.' },
        { name: 'Baltic Sea (Kaliningrad)', lat: 54.7, lon: 20.5, radiusKm: 180, severity: 'high', source: 'Russia', description: 'Kaliningrad-origin jamming affecting Finnish, Baltic and Scandinavian airspace.' },
        { name: 'Black Sea Region', lat: 44.5, lon: 34.0, radiusKm: 150, severity: 'high', source: 'Russia', description: 'Active jamming correlates with military ops in Ukraine.' },
        { name: 'Northern Finland/Norway', lat: 68.5, lon: 27.0, radiusKm: 120, severity: 'medium', source: 'Russia', description: 'Kola Peninsula area. Interference with Arctic navigation systems.' },
        { name: 'Persian Gulf', lat: 26.5, lon: 52.0, radiusKm: 100, severity: 'medium', source: 'Iran', description: 'Spoofing incidents near Strait of Hormuz. Multiple vessel diversions.' },
        { name: 'South Korea (Seoul area)', lat: 37.5, lon: 126.5, radiusKm: 80, severity: 'medium', source: 'North Korea', description: 'NK spoofing campaigns targeting aviation and maritime navigation.' },
        { name: 'Gaza/Israel', lat: 31.8, lon: 34.5, radiusKm: 90, severity: 'critical', source: 'IDF/Unknown', description: 'Widespread GPS denial. Affects entire Levant. Civilian aviation rerouting.' },
        { name: 'Donbas/Eastern Ukraine', lat: 48.5, lon: 37.5, radiusKm: 130, severity: 'critical', source: 'Russia', description: 'Combat theater jamming. All navigation systems degraded.' },
        { name: 'Red Sea (Houthi Zone)', lat: 14.0, lon: 42.5, radiusKm: 110, severity: 'high', source: 'Houthis/Iran', description: 'Ship navigation interference. Spoofing linked to drone attack guidance.' },
        { name: 'Taiwan Strait', lat: 24.0, lon: 119.5, radiusKm: 100, severity: 'medium', source: 'China/PLA', description: 'PLA exercises. GPS unreliable during tension periods.' },
      ];
    ...
    <div className="glass-panel h-full flex flex-col relative">
          ...
          <div className="grid grid-cols-3 gap-1">
            {LAYERS.map(layer => (
              <button
                ...
                style={activeLayers.includes(layer.id) ? { borderLeft: `2px solid ${layer.color}` } : {}}
              >
      ...
      <div className="absolute bottom-2 left-2 bg-void/90 backdrop-blur-sm rounded p-2 text-[9px] space-y-1 z-10 max-w-[180px]">
        <div className="text-text-muted font-mono mb-1 flex items-center justify-between">
          <span>LAYERS</span>
          <span className="text-[8px] text-text-dim">{activeLayers.length} active</span>
        </div>
        ...
        <div className="absolute top-14 left-2 bg-void/90 backdrop-blur-sm rounded p-2 text-[9px] z-10">
          ...
          {clusters.slice(0, 5).map(c => (
            <div key={c.region} className="flex items-center justify-between gap-3">
              ...
              <span className="font-mono" style={{ color: c.color }}>{c.count}</span>
            </div>
          ))}
        </div>
      )}
...
function createMarkerElement(emoji: string, color: string, title: string, scale = 1): HTMLElement {
  const el = document.createElement('div');
  el.className = 'cursor-pointer transition-transform hover:scale-125';
  el.style.cssText = `
